...
@section('content')
    ...
    <div class = "row">
        ...
        {!! ViewHelper::formGroup([
             ['type' => 'text', 'name'=>'name', 'label' => 'Name', 'element_width' => 10, 'value' => $report->name],
             ], $errors) !!}
    ...
    <div class="row">
            ...
            <div class="tab-pane" id="look-tab">
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-5">
                            {!! Form::select('template_id', $template_list, $report->template_id, ['class' => 'form-control']) !!}
                        </div>
                    </div>
                </div>
                <br />
        ...
        <div class="btn-group">
            ...
            <button class="btn btn-primary parse-json" id="btn-copy" data-target="basic">Copy</button>
        </div>
    ...
    <script>
        ...
        $('#btn-save').on('click', function() {
            ...
            else if ($.isEmptyObject(result['columns']))
            {
                ...
                $("#report-data").attr("action", "{{route('reports.update', $report->id)}}");
                $("#report-data-data").val(JSON.stringify(result, null, 2));
                $("#report-data").submit();
            }
        });
        ...
        $('#btn-copy').on('click', function() {
            ...
            else if ($.isEmptyObject(result['columns']))
            {
                ...
                $("#report-data").attr("action", "{{route('reports.copy', $report->id)}}");
                $("#report-data-data").val(JSON.stringify(result, null, 2));
                $("#report-data").submit();
            }
        });
